...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.__circularConv2DMethods(tileX=True, tileY=False, startStep=0, stopStep=3, step=1)

    def __circularConv2DMethods(
        self, tileX: bool, tileY: bool, startStep: int, stopStep: int, step: int
    ):
        modules_all = [item for item in self.cond_stage_model.named_modules()] + [item for item in self.first_stage_model.named_modules()]
        for name, module in modules_all:
            if isinstance(module, torch.nn.Conv2d):
                ...
                module._conv_forward=(
                    LatentDiffusion.__replacementConv2DConvForward.__get__(
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.gmsd_weight > 0:
            loss_gmsd=self.gmsd_loss(model_output, target)
            loss_dict.update({f'{prefix}/loss_gmsd': loss_gmsd})
            loss += (self.gmsd_weight * loss_gmsd)
